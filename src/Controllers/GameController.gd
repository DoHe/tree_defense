...
func all_enemies_gone() -> void:
	...
	if not num_enemy_spawners:
		...
		get_tree().change_scene_to_file("res://src/Levels/Level%d.tscn" % level_num)
		reset()


func set_cursor(cursor):
	var cursor_size: Vector2 = cursor.get_size()
	Input.set_custom_mouse_cursor(cursor, Input.CursorShape.CURSOR_ARROW, Vector2(cursor_size.x / 2, cursor_size.y))
...
func plant(global_pos):
	...
	match planting:
		"cherry":
			var cherry := tower_scene.instantiate()
			...
			cancel_build()
		"ice":
			var ice := tower_scene.instantiate()
			level.add_child(ice)
			ice.set_kind("ice")
			...
			cancel_build()
		"big":
			var big := tower_scene.instantiate()
			level.add_child(big)
			big.set_kind("big")
...
func update_wave_label(wave: String) -> void:
	wave_label.text = "Level %d - Wave %s" % [level_num, wave]
